...
from core.pagamento import cadastrar_cartao, buscar_cartoes_usuario, atualizar_dados_cartao, excluir_cartao_banco
...
from core.account_manager import mostrar_minha_conta
        ...
        if opcao == "1":
            ...
            if not cartoes: 
                ...
                for i, c in enumerate(cartoes,1): 
                    ...
                    print(f"{i}. {c['method_type']} | Final: **** {num_final} | Apelido: {c['card_name'] if c['card_name'] else 'Sem apelido'}")

                print("\nOpções:")
                print(" [C] Cadastrar Novo | [E] Editar Apelido | [X] Excluir cartão | [V] Voltar")
                acao = input("\nEscolha uma opção: ").upper() 
                if acao == "C":
                    cadastrar_cartao(usuario)
                elif acao == "E": 
                    idx = int(input("Digite o número do cartão que deseja editar: ")) - 1
                    if 0 <= idx < len(cartoes):
                        print("\n    ATUALIZAR DADOS DO CARTÃO   ")
                        novo_nome = input("Novo Nome do Titular: ").strip()
                        novo_num = input("Novo Número do Cartão (16 dígitos): ").strip()
                        nova_val = input("Nova Validade (MM/AA): ").strip()
                        novo_cvv = input("Novo CVV (3 dígitos): ").strip()

                        sucesso, msg = atualizar_dados_cartao(
                            usuario['id'], 
                            cartoes[idx]['id'], 
                            novo_nome, 
                            novo_num, 
                    ...
                    idx = int(input("Digite o número do cartão que deseja excluir: ")) - 1
                    if 0 <= idx < len(cartoes):
                        confirmar = input(f"Tem certeza que deseja excluir o cartão {idx+1}? (S/N): ").upper()
                        if confirmar == "S":
                            
                            sucesso, msg = excluir_cartao_banco(usuario['id'], cartoes[idx]['id'])
                            print(f"\n{msg}")
                    else:
                        print("Opção inválida.")
            ...
            print("Opção inválida! Escolha de 1 a {'7' if usuario['is_admin'] == 1 else '6'}.")
            input("Aperte Enter...")
